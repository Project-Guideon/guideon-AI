...
-- =========================================================
-- 3.5 tb_pairing_request
-- =========================================================
CREATE TABLE IF NOT EXISTS tb_pairing_request (
  pairing_id   BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  pairing_code VARCHAR(6)  NOT NULL,
  secret       VARCHAR(64) NOT NULL,
  status       VARCHAR(20) NOT NULL,
  expires_at   TIMESTAMP   NOT NULL,
  device_id    VARCHAR(50) NULL REFERENCES tb_device(device_id) ON DELETE SET NULL,
  paired_at    TIMESTAMP   NULL,
  claimed_at   TIMESTAMP   NULL,
  created_at   TIMESTAMP   NOT NULL DEFAULT NOW(),
  updated_at   TIMESTAMP   NOT NULL DEFAULT NOW(),
  CONSTRAINT uk_pairing_code UNIQUE (pairing_code)
);

COMMENT ON TABLE tb_pairing_request IS '키오스크 페어링 요청. 6자리 코드로 device와 연결.';
COMMENT ON COLUMN tb_pairing_request.pairing_code IS '6자리 페어링 코드(유니크)';
COMMENT ON COLUMN tb_pairing_request.secret IS '페어링 시크릿 해시(SHA-256 HEX)';
COMMENT ON COLUMN tb_pairing_request.status IS 'WAITING / PAIRED / CLAIMED / EXPIRED';
COMMENT ON COLUMN tb_pairing_request.expires_at IS '코드 만료 시각';
COMMENT ON COLUMN tb_pairing_request.device_id IS 'PAIRED 이후 연결된 기기 ID';
COMMENT ON COLUMN tb_pairing_request.paired_at IS '기기 연결 완료 시각';
COMMENT ON COLUMN tb_pairing_request.claimed_at IS '페어링 수락 완료 시각';

CREATE INDEX IF NOT EXISTS idx_pairing_status ON tb_pairing_request (status, expires_at);

-- =========================================================
-- 3.7 tb_mascot (site 1:1)
-- =========================================================
...
-- =========================================================
-- 3.9 tb_doc_chunk (search engine)
-- =========================================================
