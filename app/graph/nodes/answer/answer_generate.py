...
from app.graph.nodes.utils import LANG_NAMES, build_messages, append_trace_flow, build_persona_block, get_answer_language
...
def make_answer_generate_node(llm: OpenAILLM):
    ...
    def answer_generate_node(state: GraphState) -> dict:
        ...
        user_language = get_answer_language(state)
        ...
        if user_language == "ko":
            system_prompt = (
                ...
                "  - 질문에 대한 직접적인 답이 context에 없으면 반드시 '관련 정보를 찾을 수 없습니다. 더 구체적으로 말씀해 주시겠어요?'라고만 답할 것\n"
                "  - 질문 대상이 context에 없을 때 다른 관련 주제로 전환하거나 우회 설명하지 말 것\n"
                "  - 출처 문서명은 읽어주지 않아도 됨"
            )
            user_msg = (
                f"질문: {text}\n\n"
