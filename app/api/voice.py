...
# Spring FileValidator가 더 엄격하게 검증하므로 여기선 하드 상한만 적용
...
async def clone_voice(
    ...
    try:
        # 청크 단위로 임시 파일에 쓰면서 크기 상한 적용 (전체 메모리 적재 방지)
        with tempfile.NamedTemporaryFile(suffix=suffix, delete=False) as tmp:
            ...
            while True:
                ...
                if total > _MAX_UPLOAD_BYTES:
                    raise HTTPException(
                        status_code=400,
                        detail=f"파일이 너무 큽니다. {_MAX_UPLOAD_BYTES // (1024 * 1024)}MB 이하로 업로드하세요.",
                    )
                tmp.write(chunk)
        ...
        raise HTTPException(status_code=500, detail="음성 클로닝에 실패했습니다.") from exc
...
async def delete_voice(voice_id: str):
    ...
    try:
        # AsyncCartesia.voices.delete는 코루틴 — asyncio.to_thread 불필요
        await cartesia_tts._client.voices.delete(id=voice_id)
